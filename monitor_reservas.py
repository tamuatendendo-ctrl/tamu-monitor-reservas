...
def preparar_reserva(reserva):
    ...
    logger.info(
        "Consultando imovel da reserva %s...",
        reserva.get("id", "sem ID")
    )

    imovel = consultar_imovel(listing_id)

    logger.info(
        "Consultando hospede da reserva %s...",
        reserva.get("id", "sem ID")
    )
    ...
    hospede = (
        cliente.get("name")
        or (
            str(cliente.get("fName", ""))
            + " "
            + str(cliente.get("lName", ""))
        ).strip()
        or "Nao informado"
    )
...
def executar_ciclo(processadas, primeira_execucao):
    ...
    logger.info(
        "Reservas encontradas na API: %s",
        len(reservas)
    )

    if primeira_execucao:
        logger.info(
            "🟡 PRIMEIRA EXECUCAO: registrando reservas sem alertar."
        )
        ...
        logger.info(
            "✅ %s reservas registradas sem Telegram.",
            adicionadas
        )
    ...
    for reserva in reservas:
        ...
        try:
            ...
            logger.info(
                "📱 Enviando alerta para Telegram..."
            )

            enviar_telegram(
                montar_mensagem(dados)
            )
            ...
            logger.info(
                "💾 Reserva %s registrada como processada.",
                dados["reserva"]
            )
...
def main():
    ...
    logger.info(
        "Controle: %s",
        ARQUIVO_PROCESSADAS.name
    )

    logger.info(
        "Status: %s",
        ARQUIVO_MONITOR.name
    )

    processadas = carregar_processadas()

    logger.info(
        "Reservas já processadas: %s",
        len(processadas)
    )

    primeira_execucao = (
        PRIMEIRA_EXECUCAO_SILENCIOSA
        and not processadas
    )
